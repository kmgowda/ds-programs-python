...
class AVLTree():
    ...
    def leftrotate(self, cur):
        ...
        cur.left = x.right
        x.right = cur
        cur.bfactor = cur.bfactor-1-self.max(x.bfactor*-1, 0)
        x.bfactor = x.bfactor-1-self.max(0, cur.bfactor)
        return x
    ...
    def rightrotate(self, cur):
        ...
        cur.right = x.left
        x.left = cur
        cur.bfactor = cur.bfactor+1+self.max(0, -1*x.bfactor)
        x.bfactor = x.bfactor+1+self.max(cur.bfactor, 0)
        return x           
    ...
    def get_height_rec(self, cur):
        ...
        if bfactor != 0 and bfactor != 1 and bfactor != -1:
            print("AVL tree imbalance at %d [%d] actual b factor %d" %(cur.value, cur.bfactor, bfactor))
        else: 
           if cur.bfactor != bfactor:
               print("AVL tree stored b factor %d and actual b factor %d are not same at node %d" %(cur.bfactor, bfactor, cur.value))     
